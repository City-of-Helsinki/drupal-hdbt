...
test.beforeEach(async ({ page }) => {
  ...
  await page.getByLabel(translations.label_monthly_pay.fi).fill('400');
  await page.getByLabel(translations.label_vacation_money.fi).fill('16');
});

test('[Business] Fill in form and check correct calculations as business (low)', async ({ page }) => {
...
test('[Association: business] Fill in form and check correct calculations as (low)', async ({ page }) => {
...
test('[Association] Fill in form and check correct calculations as association (low)', async ({ page }) => {
  await page.getByText(translations.label_company_type_association.fi, {exact: true}).click();
