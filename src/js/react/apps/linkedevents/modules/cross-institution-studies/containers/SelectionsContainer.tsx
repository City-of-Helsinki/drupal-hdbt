...
import { languageAtom, resetFormAtom, submittedParamsAtom, updateDatesAtom, updateParamsAtom, updateUrlAtom } from '../../../store';
...
const SelectionsContent = memo(({
  ...
  if (submittedParams.has(ApiKeys.LANGUAGE) ) {
    ...
    selections.push(...selectedLanguages.map(lang => (
      <FilterButton
        key={lang.value}
        value={lang.label}
        clearSelection={() => {
  ...
  if (submittedParams.has(ApiKeys.KEYWORDS)) {
    ...
    selections.push(...selectedModes.map(mode => (
      <FilterButton
        key={mode.value}
        value={mode.label}
        clearSelection={() => {
