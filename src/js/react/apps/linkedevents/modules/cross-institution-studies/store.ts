...
import { languageAtom, pageAtom, paramsAtom, submittedParamsAtom, updateDatesAtom, updateUrlAtom } from '../../store';
...
export const initializeStateAtom = atom(null, (get, set, dateOptions: Map<string, { start?: DateTime; end?: DateTime }>) => {
  ...
  if (params.has(ApiKeys.END) && params.has(ApiKeys.START)) {
    ...
    if (start && end) {
      const matchingOption = Array.from(dateOptions.entries()).find(([_, { start: optionStart, end: optionEnd }]) => {
        console.log({...{start, end, optionStart: optionStart?.toISO(), optionEnd: optionEnd?.toISO()}});
  ...
  if (params.has(ApiKeys.LANGUAGE)) {
    ...
    const selectedLanguages = params.get(ApiKeys.LANGUAGE)
      ?.split(',')
  ...
  if (params.has(ApiKeys.KEYWORDS)) {
    const allKeywords = params.get(ApiKeys.KEYWORDS)?.split(',').map((kw) => kw.trim()) || [];
