...
const useQueryString = (urlParams: URLParams): string => {
  ...
  if (languageFilter?.bool?.filter) {
    must.push({ term: { entity_type: 'node' } });
  }

  if (urlParams?.topic?.length) {
    must.push({ terms: { [IndexFields.NEWS_TAGS]: urlParams.topic } });
  }

  if (urlParams?.groups?.length) {
    must.push({ terms: { [IndexFields.NEWS_GROUPS]: urlParams.groups } });
  }
  ...
  const query: any = { ...languageFilter };
  ...
  const result: estypes.SearchRequest = {
    from: size * (page - 1),
    query,
    sort: ['_score', { [IndexFields.PUBLISHED_AT]: 'desc' }],
    size,
  };
